...
Public Class SetUpFees
    ...
    Public Sub New(ByVal dbConnection As SqlClient.SqlConnection, ByVal CurrentSessionID As String, ByVal TransactionID_Current As String, ByVal ControlRoot_Current As String)
        ...
        If Not ERP_Type.ToUpper = "PARENT" Then Exit Sub 'no setup fee here
        ...
        Try
                ...
                For id = 0 To ProdList.Rows.Count - 1 'figure out the sku 'separate by store/description type

                    'we only deal with setup_fee here
                    If CStr(ProdList.Rows(id).Item("sku_2")).ToUpper = "SET_UP" Then
                        'Dim productid As String = ProdAll(0).Item("product_id")
                        Dim CurStore As String = CStr(ProdList.Rows(id).Item("store"))
                        Dim Updated As Boolean = False
                        '=================== UPDATE CODE ============================

                        RowFilter = "sku='" & ProdList.Rows(id).Item("sku") & "'"
                        ProdAll.RowFilter = RowFilter 'we have 4 or more rows we need to group by store

                        Dim sku_2 As String = Nothing
                        Dim ProductId As Integer = 0

                        Dim ProdIDNull = ProdAll(0).Item("product_id")
                        If Not IsDBNull(ProdIDNull) Then
                            ProductId = ProdAll(0).Item("product_id")
                        End If

                        If ProductId = 0 Then GoTo skipDescription : 
                        ...
                        ProdList.Rows(id).Item("Result") = Updated
                        If Updated Then ProdList.Rows(id).Item("DateUpdated") = Now()
                        ProdList.Rows(id).Item("InUpdateUniverse") = Not Updated
                    ...
                    'If ProductDescription(0).sku_2 = "SET_UP" Then
                    '    GetCatalogAttributesOptions("abax")
                    'End If
                    ...
                    'set up etc..
                    'catalogProductAdditionalAttributesEntity  in productdata
...
SKIP_Prod:
        ...
        End Sub
    Private Sub GetCatalogAttributesOptions(ByVal code As String)
        'Traverse catalogAttributeEntity until code = ProductDescription(0).name and return attribute_id (we will store in a list) 
        ProductCatalogAttributeEntity = MageHandler.catalogCategoryAttributeList(SessionId)
        'lets store all that stuff as list

    End Sub

    Private Sub SetCatalogProductAdditionalAttributesEntity()
        'multi data entity array associativeMultiArray in productdata as catalogProductCreateEntity
    End Sub
    Private Sub MagentoCatalogAttributesOptionsWrite(ByVal productdata As catalogProductCreateEntity, ByVal productID As Integer, ByVal storeView As String, ByVal identifierType As String, ByRef Updated As Boolean)
    ...
    End Class
    Private Class Mage_catalogProductAttributeList
        Property attribute_id As Integer
        Property code As String  'custom_colour_fee
        Property type As String  'price - text
        Property scope As String 'Store
    End Class
...
NoOption:
    ...
    Private Sub AddToMage_catalogProductAttributeList(ByRef Mage_catalogProductAttributeList_IP As List(Of Mage_catalogProductAttributeList), ByVal attribute_id As Integer, ByVal code As String, ByVal type As String, ByVal scope As String)

        Mage_catalogProductAttributeList_IP.Add(New Mage_catalogProductAttributeList() With {
